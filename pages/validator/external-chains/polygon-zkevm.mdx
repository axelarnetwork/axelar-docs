...
## Set node config
Define variable to set
```bash
# your network could be either mainnet or tetsnet, we defined this earlier
...
```
Using sed commands to update the variables, be sure you know what you are updating
```bash
# Update ZKEVM_NETWORK variable
sed -i 's@ZKEVM_NETWORK = "[^"]*"@ZKEVM_NETWORK = "'"${ZKEVM_NETWORK}"'"@' "$ZKEVM_CONFIG_DIR/.env"

# Update ZKEVM_NODE_ETHERMAN_URL variable
sed -i 's@ZKEVM_NODE_ETHERMAN_URL = "[^"]*"@ZKEVM_NODE_ETHERMAN_URL = "'"${ZKEVM_NODE_ETHERMAN_URL}"'"@' "$ZKEVM_CONFIG_DIR/.env"

# Update ZKEVM_NODE_STATEDB_DATA_DIR variable
sed -i 's@ZKEVM_NODE_STATEDB_DATA_DIR = "[^"]*"@ZKEVM_NODE_STATEDB_DATA_DIR = "'"${ZKEVM_NODE_STATEDB_DATA_DIR}"'"@' "$ZKEVM_CONFIG_DIR/.env"

# Update ZKEVM_NODE_POOLDB_DATA_DIR variable
sed -i 's@ZKEVM_NODE_POOLDB_DATA_DIR = "[^"]*"@ZKEVM_NODE_POOLDB_DATA_DIR = "'"${ZKEVM_NODE_POOLDB_DATA_DIR}"'"@' "$ZKEVM_CONFIG_DIR/.env"
```
