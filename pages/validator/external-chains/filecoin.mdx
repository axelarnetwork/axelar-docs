...
If you have an AMD Zen or Intel Ice Lake CPU (i.e 10th gen Intel Core/3rd gen Xeon or later),
...
Peek at the systemd file to make sure there are no configurations that you wish to change e.g lotus home path, logs destination and format etc.
...
```bash
sudo systemctl start lotus-daemon # using sudo might result is data being stored in /root/.lotus
```
...
{`[[axelar_bridge_evm]]
name = "filecoin"
rpc_addr = "http://127.0.0.1:1234/rpc/v1"
