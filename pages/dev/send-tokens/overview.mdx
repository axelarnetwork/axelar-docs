...
Find a list of assets, their names and their addresses: [Mainnet](../reference/mainnet-contract-addresses) | [Testnet](../reference/testnet-contract-addresses). Note that for the same asset denom,
the ERC20 symbol could be different on different chains.
For e.g., `USDC` on Ethereum is linked to `axlUSDC` on Avalanche.
