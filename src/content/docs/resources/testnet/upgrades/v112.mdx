...
Upgrade height `<TODO>` [countdown](https://testnet.mintscan.io/axelar-testnet/blocks/TODO)
...
```bash
axelard tx gov vote <TODO> yes --from validator
```


2. Wait for the proposed upgrade block, `<TODO>`. Your node will panic at that block height with a log: `{"level":"error","module":"consensus","err":"UPGRADE \"v1.1.2\" NEEDED at height: ",`. Stop your node after chain halt.
...
5. If you're a validator also restart `vald` with `v1.1.2` and `tofnd` with `v0.10.1`.
...
KEYRING_PASSWORD="pw-1" TOFND_PASSWORD="pw-2" ./scripts/validator-tools-host.sh -n testnet -a v1.1.2 -q v0.10.1
```
