...
The `v1.3.1` upgrade is required to support the Axelar Network's upgrade to [Cosmos SDK v0.5.0](https://cosmos-docs.mintlify.app/sdk/v0.50/build). This upgrade will allow for faster block times (from 5s to 1s) on Axelar as well as support for the latest security features of the Cosmos SDK.
...
5. Restart your `axelard` node with the new `v1.3.1` build as well as `ampd` with the new `v1.13.1` binaries.
6. If you're a validator also restart `vald` with `v1.3.1` and `tofnd` with `v1.0.1`. TODO confirm tofnd version
