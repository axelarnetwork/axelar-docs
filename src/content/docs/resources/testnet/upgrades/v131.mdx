...
The `v1.3.1` upgrade is required to support the Axelar Network's upgrade to [Cosmos SDK v0.50](https://cosmos-docs.mintlify.app/sdk/v0.50/build). This upgrade will allow for faster block times (from 5s to 1s) on Axelar as well as support for the latest security features of the Cosmos SDK.
...
For verifiers, a new `ampd` [upgrade](https://github.com/axelarnetwork/axelar-contract-deployments/blob/main/releases/ampd/2025-11-17-ampd-v1.13.2.md) is required to support `axelard v1.3.1`   The `ampd` upgrade must be done **after**  completing the `axelard` upgrade.

### Instructions

Restart `ampd` with the new binary after deploying axelard `v1.3.1`. The relevant `ampd` release can be found [here](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/ampd-v1.13.2).
