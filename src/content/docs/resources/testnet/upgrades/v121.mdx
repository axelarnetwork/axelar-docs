...
Instructions for the `2025-Feb-13` testnet upgrade to axelar-core `v1.2.1`.
...
## About the v1.2.1 upgrade
The `v1.2.1` upgrade is a consensus breaking release for Axelar testnet. Once live, it will upgrade the network so that AXL transaction fees are sent to a burn address, instead of distributing them to token holders.
...
2. Wait for the proposed upgrade block, `17629900`. Your node will panic at that block height with a log: `{"level":"error","module":"consensus","err":"UPGRADE \"v1.2.1\" NEEDED at height: 17629900",`. Stop your node after chain halt.
