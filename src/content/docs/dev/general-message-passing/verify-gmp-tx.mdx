...
For the Neutron blockchain for example to derive the Axelar account you pass in `channel-2`, <code>{COSMOS_GMP_ADDRESS}</code>, `neutron` to the `DeriveIntermediateSender` function.
