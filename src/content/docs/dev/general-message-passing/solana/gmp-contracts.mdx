...
The Solana Axelar [Gateway](/learn/network/flow/#gateways) is a Solana program that enables cross-chain message passing between Solana and other blockchains via Axelar. It serves as the authentication layer for General Message Passing (GMP) messages, verifying that incoming cross-chain messages are properly signed by Axelar's verifier set.


The Solana Axelar Gateway is the Solana-side implementation of Axelar's cross-chain gateway protocol. It enables:

- **Outbound Messages**: Solana programs can send messages to other chains via `call_contract`
- **Inbound Messages**: Programs can receive and validate cross-chain messages from other chains
...
The Gateway Config is the root configuration [PDA](https://solana.com/docs/core/pda) storing verifier set information 
...
A PDA tracking individual message state. This PDA is [derived](https://github.com/axelarnetwork/axelar-amplifier-solana/blob/main/programs/solana-axelar-gateway/src/state/incoming_message.rs#L19) via the [command_id](/resources/axelar-commandid/) of the message. This PDA is used to track the status of the message (approved or executed) and the hash of the message and payload.
...
A PDA tracking the status of the signature verification process. This PDA is derived via the `payload_merkle_root` of the message.
...
A PDA used for tracking verifier set epochs and hashes. This PDA is derived via the `verifier_set_hash`.
...
The [call_contract](https://github.com/axelarnetwork/axelar-amplifier-solana/blob/main/programs/solana-axelar-gateway/src/instructions/call_contract.rs) instruction defined in the Gateway, will begin the process of sending an outbound cross-chain message.

The instruction takes four parameters:

1. `destination_chain`: The name of the destination chain.
1. `destination_contract_address`: The address oo the destination where the call will be sent to.
...
1. `signing_pda`: Optional PDA signer
1. `gateway_root_pda`: Gateway configuration PDA
...
If the caller is a separate program making uses of a [CPI](https://solana.com/docs/core/cpi), you must derive a signing PDA using `CALL_CONTRACT_SIGNING_SEED` and your program ID, provide it as `Some(signing_pda)`, and invoke the instruction with `CpiContext::new_with_signer()` using the PDA's seeds so it can sign on behalf of your program.
...
The Gateway Program is also a key component in the receiving of a GMP message. The Gateway Program will be responsible for marking a message as approved and then validating that message once the destination program is executing it.
...
Once validated, the instruction marks the message as executed, preventing replay attacks. This ensures that each approved message can only be executed once by its designated recipient.
...
1. `gateway_root_pda`: The Gateway configuration PDA containing domain separator and validation data.
...
To validate messages in your destination program, use the `executable_accounts!` macro and `validate_message` helper:
...
pub struct Execute<'info> {
    // Gateway accounts - generated by the macro
    pub executable: AxelarExecuteAccounts<'info>,
    
    // Your program's accounts
    #[account(mut)]
    pub my_program_data: Account<'info, MyData>,
}
...
pub fn execute_handler(
    ...
    // Process your payload - message is now validated
    let decoded_data = decode_payload(&payload)?;
    ctx.accounts.my_program_data.update(decoded_data)?;
...
Use the `executable_accounts!` macro to generate the required account structure:
...
Emitted when verifier set is rotated.
...
Emitted when operator is transferred.
...
A full example of the sending a GMP message can be found [here](https://github.com/axelarnetwork/axelar-amplifier-solana/tree/main/programs/solana-axelar-memo).

## Support

For questions or issues:
- Open an [issue](https://github.com/axelarnetwork/support/issues) on GitHub
