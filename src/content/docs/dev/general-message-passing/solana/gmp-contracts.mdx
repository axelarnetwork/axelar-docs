...
1. `signing_pda`: An optional PDA signer (required when caller is a program)
1. `gateway_root_pda`: [Gateway configuration](/dev/general-message-passing/solana/gmp-contracts/#gateway-config) PDA
1. `event_authority`: The Gateway's event authority account (for event emission)
1. `program`: The Gateway program account
...
let accounts = CallContract {
    ...
    event_authority: gateway_event_authority.to_account_info(),
    program: gateway_program.to_account_info(),
};
```
...
<Callout emoji="💡">
    **Note:** Unlike the `approve_message` instruction which is called by Axelar's relayer, the `validate_message` instruction must be called by your destination program as part of its execute handler. This is typically done using the [executable_accounts](/dev/general-message-passing/solana/gmp-contracts/#executable-macro) macro and `validate_message` helper function provided by the Gateway SDK. See the [Send & Receive Messages](/dev/general-message-passing/solana/gmp-messages/) guide for a complete walkthrough.
</Callout>
...
The [executable_accounts!](https://github.com/axelarnetwork/axelar-amplifier-solana/blob/main/programs/solana-axelar-gateway/src/executable.rs) macro generates the account structure your destination program needs to receive and validate Axelar GMP messages. For a step-by-step guide on using this macro, see [Send & Receive Messages](/dev/general-message-passing/solana/gmp-messages/).

<Callout emoji="💡">
The `executable_accounts!` macro is a convenience helper. If you need more control, you can copy the macro's generated code from the [source](https://github.com/axelarnetwork/axelar-amplifier-solana/blob/main/programs/solana-axelar-gateway/src/executable.rs) and customize it directly.
</Callout>

The macro generates an `AxelarExecuteAccounts` struct with the following accounts:

- `incoming_message_pda` — The approved message PDA (mutable, derived from command ID)
- `signing_pda` — Validates the message signer (derived from command ID)
- `gateway_root_pda` — Gateway configuration PDA
- `event_authority` — Gateway event authority
- `axelar_gateway_program` — Reference to the Gateway program

The `validate_message` helper function performs a CPI to the Gateway to verify the message and mark it as executed:
...
pub fn execute_handler(
    ...
    // Validate the message — this performs CPI to the Gateway
    validate_message(
        ctx.accounts,
        message,
        &payload,
...
|------|-------------|---------|
| `b"gateway"` | Gateway program ID | Gateway configuration PDA |
| `b"incoming message"` + command_id | Gateway program ID | Incoming message PDA tracking approval/execution status |
| `b"gtw-validate-msg"` + command_id | Gateway program ID | Signing PDA for message validation CPI |
| `b"gtw-call-contract"` | Your program ID | Signing PDA authorizing outbound `call_contract` calls |
| `b"ver-set-tracker"` + verifier_set_hash | Gateway program ID | Tracks verifier set epochs |
| `b"gtw-sig-verif"` + payload_merkle_root | Gateway program ID | Tracks signature verification progress |

### ITS Seeds

| Seed | Derived From | Purpose |
|------|-------------|---------|
| `b"axelar-its-token-authority"` | Your program ID | Authority for the ATA holding received interchain tokens. Sign with this to spend. |
| `b"interchain-transfer-execute"` + destination_program_id | ITS program ID | Signer proving ITS invoked your program via CPI |
| `b"interchain-token-service"` | ITS program ID | ITS root configuration PDA |
| `b"token-manager"` + its_root_pda + token_id | ITS program ID | Token manager PDA for a given token |
