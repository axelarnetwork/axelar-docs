...
1. `sender`: The [Incoming message](/dev/general-message-passing/solana/gmp-contracts/#incoming-message) PDA that must be in `approved` status.
...
The [addGas()](https://github.com/axelarnetwork/axelar-amplifier-solana/blob/main/programs/solana-axelar-gas-service/src/instructions/add_gas.rs) instruction is used to add additional gas to a cross-chain transaction after it has been paid for in the an insufficient amount of gas was sent to cover the cross-chain transaction.

The `payGas()` instruction takes the following parameters:
