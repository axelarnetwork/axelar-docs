...
import { COSMOS_GMP_ADDRESS } from "/src/config/variables";
...
To send a message from an EVM chain to a Cosmos CosmWasm contract, the payload must be encoded before calling the [standard](/dev/general-message-passing/gmp-messages) `payNativeGasForContractCall` and `callContract` methods (or [token-including equivalents](/dev/general-message-passing/gmp-tokens-with-messages)). You must also specify the destination method you are calling.
...
To send a message from Native Cosmos Go code, you must send a message to the Axelar GMP Account (<code>{COSMOS_GMP_ADDRESS}</code>) via IBC.
...
To send a message from CosmWasm Rust code, you must send a message to the Axelar Gateway (<code>{COSMOS_GMP_ADDRESS}</code>) via IBC.
...
1. That the packet came from Axelar (verify IBC channel; Axelar GMP sender account).
   ...
   - **For Consensus**
     ...
     Verify (channel & sender == address of our GMP account = <code>{COSMOS_GMP_ADDRESS}</code>.
