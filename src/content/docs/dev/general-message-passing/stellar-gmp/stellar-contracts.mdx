...
<Callout emoji="💡">
Note: In Stellar, you don't need to pass the `env` explicitly when writing contract functions; it is automatically provided to the contract entry points
</Callout>
...
The `validate()` function on the `Executable` will trigger the `validate_message()` that is [defined](https://github.com/axelarnetwork/axelar-cgp-stellar/blob/main/contracts/axelar-gateway/src/contract.rs#L128) on the `Gateway`.
