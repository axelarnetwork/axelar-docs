...
Axelar's [Interchain Token Service (ITS)](https://www.axelar.network/its) is live on the Sui blockchain to allow for the integration of coins on Sui to be sent to/from [other ecosystems](https://axelarscan.io/resources/chains) connected to the Axelar Network. ITS allow teams to deploy fresh new fungible [Interchain Tokens](https://github.com/axelarnetwork/axelar-cgp-sui/blob/main/move/interchain_token/sources/interchain_token.move) as well as integrate custom tokens that want to go cross-chain. ITS is already live on many different EVM and non-evm chains so you can now send your Sui coins to and from those chains.
...
Before you can register your coin however, you first must create a [coin-manager]("coin-manager") and a [coin-info]("coin-info") for your coin.
...
When tokens are sent to Sui, the [Relayer Discovery](https://github.com/axelarnetwork/axelar-cgp-sui/blob/main/move/interchain_token_service/sources/discovery.move) module will trigger the `receive_interchain_transfer()` function on the Sui ITS module. 

The function takes 4 parameters: 
1. `its`: The ITS module that will be updated once the new coin is registered
1. `ticket`: The [ticket](interchain-transfer-ticket) representing the token transfer
1. `clock`: A [clock](https://docs.sui.io/references/framework/sui/clock) module that provides the time of the transfer
...
See [here](https://github.com/axelarnetwork/axelar-contract-deployments/blob/main/sui/its-example.js#L200) for an example of how deploy a new capped manangers.
...
### Set Flow Limit
To set a specific [flow limit](#flow-limit) amount you can trigger the `set_flow_limit()` function.

The function takes three parameters:

1. `coin_management` (`self`): The module that will be updated once the `flow_limit` is set
...
1. `flow_limit`: An optional unsigned 64-bit integer representing the new flow limit
...
#### Treasury Capability
A special permission (represented as a `TreasuryCap`) that, when present, enables minting and burning of tokens. A CoinManagement instance created with this capability can adjust the token supply.
