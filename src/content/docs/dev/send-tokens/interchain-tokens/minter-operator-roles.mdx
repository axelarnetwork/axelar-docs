...
- The ITS is initially set as the minter for Native Interchain Tokens
...
In the ITS, operators are often set during token manager deployment:
...
The `linkParams` is often used to specify the operator address for the token manager.
...
2. **Cross-Chain Transfers**: For mint/burn token managers, the minter creates tokens on the destination chain equivalent to what was burned on the source chain.
...
1. **Flow Control**: Operators can set limits on how many tokens can flow in or out of a chain in a given period.
2. **Security Governance**: Operators can implement security measures to protect against attacks.
3. **Service Authorization**: Operators authorize the ITS to interact with token managers.
