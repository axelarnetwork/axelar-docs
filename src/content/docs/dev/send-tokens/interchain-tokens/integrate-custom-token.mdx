...
Note: This function is called from the Interchain Token Factory rather than directly from the ITS because the Factory provides additional utility functions and standardized token ID computation logic.
...
I'll update the "Link Custom Token with the Interchain Token Factory" section to provide more detailed information and include a reference to an example transaction. Here's the revised content:
