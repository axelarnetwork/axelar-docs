...
This function allows the [operator](/dev/send-tokens/interchain-tokens/minter-operator-roles/#operator-role) or an approved flow limiter for the token to limit how many tokens can flow in or out over time.

#### Setting Flow Limits

Flow limits are configured per token and can be set by the contract operator or an approved flow limiter for the token:

```rust
// Set a flow limit for a token
client.set_flow_limit(&caller, &token_id, &Some(1000_000));

// Remove flow limit (disable flow limiting)
client.set_flow_limit(&caller, &token_id, &None);

// Freeze token transfers (set limit to 0)
client.set_flow_limit(&caller, &token_id, &Some(0));
```

#### Authorization

- **Operator or Flow Limiter**: Flow limits can be set by the contract operator or by an approved flow limiter for the token
