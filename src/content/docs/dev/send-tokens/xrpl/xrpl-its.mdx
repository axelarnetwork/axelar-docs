...
async function interchainTransfer(config, wallet, client, chain, options, args) {
    // encodeITSDestination produces the canonical raw-bytes hex for the destination
    // chain type; hex() then wraps it for XRPL memo transport.
    const destinationAddress = encodeITSDestination(config.chains, args.destinationChain, args.destinationAddress);

    // The gas fee is denominated in the same token as the transfer: parseTokenAmount
    // yields a drops string for XRP, or a decimal value for IOUs.
...
The script converts `--gasFeeAmount` into the format the memo requires (a `u64` drops string for XRP, a decimal string for IOUs), so you pass it in the same denomination as the transferred token. See [Encoding `gas_fee_amount`](#encoding-gas_fee_amount) for the underlying memo format.
...
```bash
node xrpl/interchain-transfer.js -e testnet -n xrpl XRP 1 xrpl-evm 0x312dba807EAE77f01EF3dd21E885052f8F617c5B --gasFeeAmount 0.1
```

`--gasFeeAmount` is denominated in XRP (`0.1` = 0.1 XRP); the script converts it to the drops value carried in the memo.
