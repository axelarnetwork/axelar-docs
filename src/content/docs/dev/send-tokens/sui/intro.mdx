...
If you do not have any metadata available for your coin then you will need to register your token via the `register_coin_from_info()` function. The function returns a new [TokenId](#token-id) and takes five parameters:
...
If you do have the metadata available then you can register the coin via `register_coin_from_metadata()` function. The function returns a new [TokenId](#token-id) and takes three parameters:
...
The `register_coin_from_metadata()` returns a [MessageTicket](/dev/general-message-passing/sui/gmp-contracts/#message-ticket) and takes the following two parameters:
...
1. `deployer` The the deployer’s [channel](/dev/general-message-passing/sui/gmp-contracts/#channel) capability. Used to derive the custom [tokenId](#token-id) with the `salt` and `chain_name`, so only that channel’s holder can link that custom token.
...
1. `link_params` A 32 byte representation of an address to be used as the operator on the destination chain.
...
Note: A flow limit of `0` corresponds to no flow limit set at all. If you wish to remove your flow limit it should be set to 0. To halt transfers of your token in/out of Sui you can set the flow limit to a near `0` value, effectively halting cross-chain transfers.
...
1. `token_id`: The id of the token that will receive the new operator.
...
A [Flow Limit](/dev/send-tokens/interchain-tokens/flow-limit/) represent the volume of a coin that can be transferred in/out of Sui via ITS. This limit plays a critical role in maintaining network integrity and security. When coins are sent out of Sui. The flow limit logic can be found in its own [Flow Limit Module](https://github.com/axelarnetwork/axelar-cgp-sui/blob/main/move/interchain_token_service/sources/types/flow_limit.move). The duration of a `flow_limit` is set to an `epoch`. An `epoch` lasts for six hours. It is calculated as follows:
