...
The GMP API has several available endpoints to interact with pending tasks.
...
| **Task Item Params** | **Type** | **Description** |
| --- | --- | --- |
| `id` | string | UUID of the current task |
| `timestamp` | string | Timestamp of task creation |
| `type` | [TaskType](/dev/amplifier/gmp-api/#tasktype) | The type of the underlying task |
| `task` | [Task](/dev/amplifier/gmp-api/#task) | The content of the underlying task pointed to by `type` |
