...
Once triggered the [linking message](https://github.com/commonprefix/axelar-amplifier/blob/main/contracts/interchain-token-service/src/primitives.rs#L89) is sent to [ITS Hub](/dev/amplifier/its-hub/introduction/), which will process the linking on the destination chain.

### Deploy Remote Token

The `deploy_remote_token()` function is used to send a cross-chain [GMP message](/dev/general-message-passing/overview/) to deploy a token on a remote chain. 
...
1. `config`: The [configuration](#gateway-config) for the XRPL Gateway.
1. `nexus_client`: The client for the [Nexus](https://github.com/commonprefix/axelar-amplifier/tree/main/packages/axelar-core-std/src/nexus) contract.
1. `token`: The [token](#xrpl-token) being registered
1. `destination_chain`: The name of the destination chain that the token will be linked to.
1. `token_metadata`: The [token metadata](#token-metadata) for the token being registered. 
...
Once the function is triggered it will send a [DeployInterchainToken](https://github.com/commonprefix/axelar-amplifier/blob/main/contracts/interchain-token-service/src/primitives.rs#L61) message to ITS Hub, which will process the deployment on the destination chain.

## Gas accounting

When sending cross-chain transactions users must specify a `gas_fee_amount` to be used to cover the relaying costs. Those extra funds that are sent to cover the fees are stored on the XRPL Multisig account. The Gateway contract is then responsible for tracking the amount of funds that the XRPL multisig  has received so that the relayer can at some point claim those tokens. 
...
The `InterchainTransferMessage` is performed by an XRPL user to initiate an interchain transfer call.
...
The `CallContractMessage` is performed by an XRPL user to initiate a contract call.
...
Metadata for a token. It contains two fields:
