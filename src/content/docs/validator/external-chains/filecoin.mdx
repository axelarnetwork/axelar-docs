...
<Callout title="Forest Chain Indexer">
Forest’s chain indexer is disabled by default, which causes EVM RPC methods like `eth_getTransactionReceipt` to return errors (`indexer disabled`). Vald relies on indexed receipts to see vote transactions. To enable:
...
You should eventually (approx 5-10 minutes) see syncing messages in the logs:
