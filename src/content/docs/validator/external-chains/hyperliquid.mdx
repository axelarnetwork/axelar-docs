...
Type=simple
User=$USER
ExecStart=$HOME/hl-visor run-non-validator --serve-eth-rpc --replica-cmds-style=recent-actions
Restart=on-failure
[Install]
...
<Callout type="info">
To minimize the amount of disk space used by the node, you can add the [--serve-eth-rpc](https://github.com/hyperliquid-dex/node?tab=readme-ov-file#evm) and `--replica-cmds-style=recent-actions` flags. These flags will allow your node to record the EVM and prune the rest of the logs that may be less relevant for running the Verifier.
</Callout>
