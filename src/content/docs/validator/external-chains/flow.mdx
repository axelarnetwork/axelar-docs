...
<tab-item title="Mainnet">
        ```bash
        ./boot-tools/transit pull -b ./bootstrap -t mainnet-${version-label} -r access --concurrency 10 --timeout 30m
        ```
...
<tab-item title="Testnet">
        ```bash
        ./boot-tools/transit pull -b ./bootstrap -t testnet-${version-label} -r access --concurrency 10 --timeout 30m
        ```
...
1. `access-node`: ensure it's fully synced in terms of block height before proceeding.
    - The latest chain height can be queried through Flow CLI (`flow blocks get latest - mainnet`) and also available on the Flow [block explorer](https://www.flowscan.io/) or [testnet](https://testnet.flowscan.io/)
2. `gateway`
