...
<tabs>
    ...
    <tab-item title="Flow CLI created account">
        ...
        Update the amount of FLOW in the above JSON to specify how much gas to send to the COA EVM account. This step requires a configured `flow.json` to be created to run successfully, see [account creation step 3](./account-creation)
    </tab-item>
