...
<Callout type="error" emoji="⚠️">
  Replace `$USER` and path to `geth` in the config below. For Goerli Testnet,
  add the `--goerli` flag to the `geth` command.
</Callout>
...
<tab-item title="Mainnet">
 <pre><code>geth attach ipc:/root/.ethereum/geth.ipc
eth.syncing</code></pre>

</tab-item>
<tab-item title="Goerli Testnet">
 <pre><code>geth attach ipc:/root/.ethereum/goerli/geth.ipc
eth.syncing</code></pre>
...
<Callout emoji="📝">Goerli testnet chain name is `ethereum-2`</Callout>

<tabs>
  <tab-item title="Mainnet">
    ```bash [[axelar_bridge_evm]] name = "Ethereum" rpc_addr = "http://IP:PORT"
    start-with-bridge = true 
```
  </tab-item>
  <tab-item title="Goerli Testnet">
    ```bash [[axelar_bridge_evm]] name = "ethereum-2" rpc_addr =
