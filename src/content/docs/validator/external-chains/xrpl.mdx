...
- Under the `[node_db]` stanza, update `online_delete` (keep the rest of the parameters as is):
```
[node_db]
...
7. Configure the `[server]` stanza in `rippled.cfg` securely according to your needs and make sure to prevent unauthorized access to your node.
