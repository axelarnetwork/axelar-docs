...
If your node runs into an issues (such as an `AppHash` error) the node orchestration should perform a `rollback` on crash **before restarting the node** to auto-recover from such errors. The rollback can be performed by running `axelard rollback`. 