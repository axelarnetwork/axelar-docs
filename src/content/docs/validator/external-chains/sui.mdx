...
### Restore from Snapshot and setup config files

<tabs>
  <tab-item title="Mainnet" class="bleed">
    ```bash 
    mkdir -p /opt/sui/config/ cd /opt/sui/config/
    curl -fLJO
...
```
  ...
  <tab-item title="Testnet" class="bleed">
    ```bash mkdir -p /opt/sui/config/ cd /opt/sui/config/ 
...
### Restore using Formal Snapshots
See the [official documentation](https://docs.sui.io/guides/operator/snapshots#restoring-using-formal-snapshots) on this step.
