...
<Callout emoji="📝">
  Note: In the service file below you need to replace `$USER` and path to
  `opera`, depending on your system configuration.
</Callout>
...
<Callout type="error" emoji="⚠️">
  If you would like to run a node on the Testnet instead, you need to replace
  `--genesis /root/go-opera/build/testnet-6226-pruned-mpt.g` in the
  configuration above.
</Callout>
