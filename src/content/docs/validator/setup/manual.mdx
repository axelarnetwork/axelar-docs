...
- [Mainnet](/resources/contract-addresses/mainnet/)
- [Testnet](/resources/contract-addresses/testnet/)
...
<Callout type="warning" emoji="⚠️">
  Protect your keyring password: The following instructions instruct you to
  store your keyring plaintext password in a file on disk. This instruction is
  safe only if you can prevent unauthorized access to the file. Use your
  discretion---substitute your own preferred method for securing your keyring
  password.
</Callout>
...
<tabs>
<tab-item title="Cosmostation">
 ```bash
axelard tendermint unsafe-reset-all
cd $HOME/.axelar/
LATEST_HEIGHT=\`curl -s https://snapshot.mainnet.cosmostation.io/axelar/latestheight\`
wget -O - https://snapshot.mainnet.cosmostation.io/snapshot/axelar/snapshot_${LATEST_HEIGHT}.tar.zst | tar -I 'zstd -d' -xf -
cd $HOME
```
...
axelard tendermint unsafe-reset-all
cd $HOME/.axelar/
URL=\`curl -s https://snapshot.ibs.team/snapshots-index.json | jq -r '.chains[] | select(.id=="Axelar") | .backends.goleveldb.latest.url'\`
wget -O - $URL | tar -I 'zstd -d' -xf -
cd $HOME
```
...
See [Support external chains](/validator/external-chains/overview/).
