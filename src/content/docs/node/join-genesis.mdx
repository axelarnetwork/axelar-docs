...
<Callout emoji="💡">
  Tip: These instructions synchronize your Axelar node from scratch using the Axelar peer-to-peer network. You can synchronize your node more quickly by downloading a recent snapshot of the blockchain as per [Quick sync](/node/join/).
</Callout>
...
<tabs>
<tab-item title="Mainnet">
 <pre><code language="bash">./scripts/setup-node.sh -n mainnet -a v0.10.7</code></pre>
</tab-item>
<tab-item title="Testnet">
 <pre><code language="bash">./scripts/setup-node.sh -n testnet -a v0.13.6</code></pre>
</tab-item>
