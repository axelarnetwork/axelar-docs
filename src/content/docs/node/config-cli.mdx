...
Example: you can use the CLI to send `uaxl` tokens to another account on Axelar via the command

```bash
axelard tx bank send my_account {DEST_ADDR} {AMOUNT}uaxl --from my_account
```
...
```bash
echo $KEYRING_PASSWORD | ~/.axelar_testnet/bin/axelard tx bank send validator {DEST_ADDR} {AMOUNT}uaxl --from validator --gas auto --gas-adjustment 1.5 --chain-id axelar-testnet-lisbon-3 --home ~/.axelar_testnet
```

Let us explain the changes:

- **Keyring password.** `echo $KEYRING_PASSWORD` : Commands that use `tx` to post a signed transaction to the network require a password to unlock the keyring. For simplicity documentation elides the keyring password. See [Keyring backend](/node/keyring/) for more info.
