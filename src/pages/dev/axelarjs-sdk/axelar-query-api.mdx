...
```ts
   ...
   * @param gasLimit An estimated gas amount required to execute `executeWithToken` function. Equivalent to the execution gas cost on the destination chain
   * @param gasMultiplier A multiplier used to create a buffer above the calculated gas fee, to account for potential slippage throughout tx execution, e.g. 1.1 = 10% buffer. Supports up to 3 decimal places. Default is `auto`.
   * @param minGasPrice (Optional) A minimum value, in wei, for the gas price on the destination chain that is used to override the estimated gas price if it falls below this specified value. Default is `0`.
