...
<Callout emoji="ℹ️">
  ...
  in some of the example contracts upon compilation. This opcode is currently only
  available on the Ethereum blockchain and its testnets. Any EVM blockchain that
  is not Ethereum may throw an error on deployment if the examples contracts being
  used are running Solidity version `0.8.20` or higher. To avoid any errors on non
  Ethereum chains make sure your Solidity code is running version `0.8.19` or below.
</Callout>
