...
1. The Axelar network confirms the call and converts the paid gas from the source chain's native token into the destination chain's native token.  
...
![simple-gmp-diagram.svg](/images/simple-gmp-diagram.svg)
...
1. The destination application contract uses the `payload` to [_execute](https://github.com/axelarnetwork/cgp-spec/blob/main/solidity/contracts/executable/AxelarExecutable.sol#L23) it's logic.