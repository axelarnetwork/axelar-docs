...
Once you are running `ampd` and `tofnd`, you can onboard a chain to the Amplifier by instantiating the `gateway`, `voting verifier`, and `multisig prover` contracts. **This requires approval by the Interop Labs team.**

You can either instantiate the existing deployed contracts on the DevNet via their `code_id`s, or build and store these contracts from the `axelar-examples` repo. A code_id is a CosmWasm identifier of a stored contract that can be instantiated.

#### Option 1: Build and store the contracts yourself
