...
To connect your chain to the Axelar network via the Interchain Amplifier, you'll need to build or instantiate the following four smart contracts along with a [message relayer](/dev/amplifier/chain-integration/introduction#message-relayer) service:
...
- The contract must implement the [Axelar Amplifier Gateway](https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/tree/main/contracts/gateway).
- EVM chains can re-use the existing [EVM Gateway](https://github.com/axelarnetwork/axelar-cgp-solidity/blob/main/contracts/AxelarGateway.sol).
