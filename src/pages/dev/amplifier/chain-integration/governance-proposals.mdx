...
axelard tx gov submit-proposal store-instantiate [path to verifier contract as wasm binary or gzip] \
    ...
    --no-admin \
    --node $RPC \
    --code-source-url "https://github.com/axelarnetwork/axelar-amplifier/tree/service-registry-v0.4.1/contracts/voting-verifier" \
    --builder "cosmwasm/optimizer:0.16.0" \
    --code-hash "4e6a37e923a48d97fa85d03911728ceee81490f4c869e64eb2bd68152bfc6448"
```

The `--code-source-url`, `--builder`, and `--code-hash` flags are currently mandatory when submitting a `store-instantiate` governance proposal.

- `--code-source-url` -- A string containing the URL to the GitHub repo pointing to the Axelar contract being instantiated.
- `--builder` -- A string representing the Docker image for the optimizer that was used to build the bytecode, such as `"cosmwasm/workspace-optimizer:0.16.0"`.
- `--code-hash` -- The SHA hash of the bytecode. You can eithr get this as part of the optimizer output when building the bytecode, or by running `shasum -a 256 [path to your .wasm file]`.
</tab-item>
...
axelard tx gov submit-proposal store-instantiate [path to gateway contract as wasm binary or gzip] \
    ...
    --no-admin \
    --node $RPC \
    --code-source-url "https://github.com/axelarnetwork/axelar-amplifier/tree/service-registry-v0.4.1/contracts/gateway" \
    --builder "cosmwasm/optimizer:0.16.0" \
    --code-hash "c2c627899e00ac9ee2631b22c13866078e200774e15931d565c7b9c40cab9d53"
```
...
axelard tx gov submit-proposal store-instantiate [path to prover contract as wasm binary or gzip] \
    ...
    --admin="axelar1zlr7e5qf3sz7yf890rkh9tcnu87234k6k7ytd9"
    --node $RPC \
    --code-source-url "https://github.com/axelarnetwork/axelar-amplifier/tree/service-registry-v0.4.1/contracts/multisig-prover" \
    --builder "cosmwasm/optimizer:0.16.0" \
    --code-hash "bb1b731be9d900c6a8535d3c6007e15b606ea302d361007a5c50021d0adcd07f"
```
