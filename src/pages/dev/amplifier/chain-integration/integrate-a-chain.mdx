...
1. Instantiate the multisig prover:
    ...
    <Callout>
    The `domain_separator` is a 32-bit length byte array derived from the hash of the chain name, router address, and Axelar chain ID.
    </Callout>
...
Every new chain will need a verifier, which requires running `ampd` and `tofnd`. See [Become an Amplifier Verifier](/validator/amplifier/verifier-onboarding) for detailed instructions on how to do this on your machine.
