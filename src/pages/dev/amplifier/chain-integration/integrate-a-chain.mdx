...
1. Instantiate the gateway. Add your own wallet address after the `--admin` command:

    ```bash
    axelard tx wasm instantiate [gateway CODE ID] '{"verifier_address":"axelar1x3960tw9cml6xsqtvzt4gmw3scauaxdd83rhs9dmlpjfjf9z9s7qjx8g0j","router_address":"axelar1fmsa0f5rs5jyauama96k0umzkktemqawrtn28zrjc0rvx47f68fqckxwuj"}' --from wallet --chain-id devnet-amplifier --gas auto --gas-adjustment 1.5 --gas-prices 0.007uamplifier --keyring-backend test --node http://devnet-amplifier.axelar.dev:26657 --keyring-backend test --label test-gateway --admin [your wallet address]
    ```
