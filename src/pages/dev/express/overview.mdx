...
Currently, the [Squid](https://www.squidrouter.com/) and [Sugarcane](https://www.sugarcane.app/) protocols are both operating with Axelar Express. Transactions that take place with Squid are significantly faster as a result. For example this [non-express transaction](https://axelarscan.io/gmp/0xc3653cda8535c59e0418b57250c504528a493dd5556fc4cb71b80f8c0a867e62:2) from Linea to Polygon took 28 minutes to execute. A transaction using express executes from the same two chains was able to execute in just [18 seconds](https://axelarscan.io/gmp/0x4dbf091ebf31157bf73995c0fb473f115888bd1f11793359807d543ada03f021:11) via Axelar Express. 
...
2. Once liquidity is provided the contract must inherit from either the [AxelarExpressExecutable](https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/blob/main/contracts/express/AxelarExpressExecutable.sol) or the [AxelarValuedExpressExecutable](https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/blob/main/contracts/express/AxelarValuedExpressExecutable.sol). Liquidity is provided for assuming the risk of an express transaction. In the event a transaction executes on the destination chain and a chain reorg occurs on the source chain then the funds will be lost (see more in risks section). The entity providing the liquidty will assume the risks for onboarding with express.
...
To allow for faster transactions. Express transactions execute on the destination chain before finalizing on the source chain. To do so Axelar Express transactions use a separate Express Relayer, the relayer will charge $1 per express call.
...
TODO
Express Msg vs. Express token
-msg is riskier (see squid) as the way logic is executed is different
-token simply requires assuming risk of reorg on src chain external team can provide tokens to mitigate the risk

B Quesitons
1. Review CommandId fix 
Basically, what I was suggesting is to not pass messageId anymore in the payload itself, and compute it dynamically using this internal method on the destination
https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/blob/main/contracts/express/ExpressExecutorTracker.sol#L20
2. Express vs Express Executable

What we have today: [relayer is loaning token that gets funded]
1. Easily Extendible -> Express Executable -> express gmp call with gateway tokens. current model is for squid we take the risk for providing that liquidity but for other partners they can provide us the tokens they want to expresss. we can have dedicated relayer for them
2. We can do GMP calls with ITS tokens. Relayer would need to be configured to support these. (No process on how to onboarded yet, we need to figure this out)
3. Arbitrary GMP calls where value of payload is what matters (i.e. messageId) Axelar valued express excutable exposes the payload relayer will need to be confiugred foer hthis
(review parrot) (ITS uses this, value is in the payload). There is an arbitrary token which were using to price the other tokens which the relayer does not have access to.

^tokens are expressed at a risk

What we don't have
1. fast finality where validator confirmations can be configured