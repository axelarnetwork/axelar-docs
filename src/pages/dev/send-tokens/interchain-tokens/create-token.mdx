...
<Callout emoji="⭐">
  ...
  the [`axelar-configs` repo](https://github.com/axelarnetwork/axelar-configs), install dependencies, and `npm run wizard` on the command line to
...
<Callout emoji="🚨">
  We're making some improvements to the custom token manager flow, which requires temporarily
  disabling Token Manager deployments. We're hoping to have it live soon.
</Callout>
