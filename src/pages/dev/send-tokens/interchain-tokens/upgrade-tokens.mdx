# Integrate existing tokens with the Interchain Token Service
...
If you already have a custom-built ERC-20 token on one or more blockchains, you can integrate it with the Interchain Token Service (ITS) by deploying [Token Managers](/dev/send-tokens/glossary/#token-manager). There are a number of different Token Manager types to choose from, depending on how you want the token to be integrated to ITS.
...
You can also do this directly via the `InterchainTokenService.sol` using the `InterchainTokenFactory.sol` to register canonical tokens and deploy remote canonical tokens.
