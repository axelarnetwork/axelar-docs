...
The portal offers two mains path. You can either deplor a fresh new token on multiple chains at once or you can connect a pre-existing token to ITS and then once connected deploy interchain tokens on other blockchains to be bridgeable with your token that you connected (and with eachother).
