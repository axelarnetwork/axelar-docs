...
Once the token has been deployed on multiple chains, you'll want to deploy a [Mint/Burn](../reference/glossary/#mintburn) [Token Manager](../reference/glossary/#token-manager) for each chain. You can do this by calling `deployCustomTokenManager` on the Interchain Token Service (or `deployRemoteCustomTokenManager` for remote chains), and then set the [distributor](../reference/glossary/#distributor) on your token to be the new Token Manager. This will give the Token Manager permissions to mint and burn tokens on your behalf as tokens move between chains.
...
If you have an ERC-20 token on a single chain, and you'd like a wrapped & bridgeable version to be available on other chains, you can register your ERC-20 as a [Canonical Token](../reference/glossary/#canonical) with the Interchain Token Service. Each token can only be registered a single time as a canonical chain.
...
One of the features of the TokenManager which can be configured is that of the [Operator](../reference/glossary/#operator). The Operator is able to manage the flow rates of tokens between chains by calling `setFlowLimit` on the Interchain Token Service. 
