# Becoming an Amplifier Worker (aka Validator)
...
By running a worker for a chain integration with Axelar, you will be responsible for voting on the validity of transactions on the Axelar network for a given chain.
...
- If you are running tofnd for the first time, a worker key will be generated for you automatically.
...
### Bond your worker
Bonding your worker registers it as a voting worker as part of the pool. Bonded workers will stay bonded until they unbond, or are removed by governance.

1. Query your worker address
...
1.  After your address has been funded, bond your worker
    (Simply copy and paste the command without altering the arguments.)
    (Can skip if you reuse the worker key)
...
1. Validators verify the source transaction
    ```bash
