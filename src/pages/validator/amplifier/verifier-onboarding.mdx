...
- **Option 1: Docker** -- This is the easiest way to get started. You will need to [install and use Docker](https://docs.docker.com/engine/install/) to set up `tofnd` and `ampd`.
...
Download the [`tofnd` binary](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/ampd-v0.4.0) depending on the type of machine you have:
    - [Linux](https://github.com/axelarnetwork/tofnd/releases/download/v0.10.5/tofnd-linux-amd64-v0.10.5)
    - [Apple Silicon Mac](https://github.com/axelarnetwork/tofnd/releases/download/v0.10.5/tofnd-darwin-arm64-v0.10.5)
    - [Intel Mac](https://github.com/axelarnetwork/tofnd/releases/download/v0.10.5/tofnd-darwin-amd64-v0.10.5)
...
Download the [`ampd` binary](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/ampd-v0.4.0) depending on the type of machine you have:
    - [Linux](https://github.com/axelarnetwork/axelar-amplifier/releases/download/ampd-v0.4.0/ampd-linux-amd64-v0.4.0)
    - [Apple Silicon Mac](https://github.com/axelarnetwork/axelar-amplifier/releases/download/ampd-v0.4.0/ampd-darwin-arm64-v0.4.0)
    - [Intel Mac](https://github.com/axelarnetwork/axelar-amplifier/releases/download/ampd-v0.4.0/ampd-darwin-amd64-v0.4.0)

### Option 2: Use Docker

From [Docker](https://hub.docker.com/r/axelarnet/axelar-ampd), pull and run the `0.4.0` version of `ampd` on the `amd64` platform:

```bash
docker pull axelarnet/axelar-ampd:v0.4.0 --platform amd64
```

### Option 3: Clone the `axelar-amplifier` repository
...
All instructions in this tutorial will assume you have a binary in your `PATH` named `ampd`. If you don't want to add `ampd` to your `PATH`, you can refer to the binary directly in every command in this tutorial, replacing `ampd` with `~/ampd-linux-amd64-v04.0`.

For example, to determine the verifier address without the `ampd` alias, you would run:

```bash
~/ampd-linux-amd64-v04.0 verifier-address
```
