...
## Check if your verifier is in an active verifier set

Query the prover to get the active verifier set. From here, you can check to see if your verifier is in the set:

```bash
axelard q wasm contract-state smart $PROVER_AVALANCHE '"get_verifier_set"' --node $RPC
```

Active verifier sets are the same as signers unless there is a verifier set update in progress, because the prover requires a few extra steps before active verifier sets can be confirmed as signers.
