...
We provide two paths to setup and run a verifier:
1. **Docker**: This is the easiest way to get started. You will need to [install and use Docker](https://docs.docker.com/engine/install/)
2. **Binaries**: This method requires you to install the `ampd` binary on your machine. You will also need to install and run `tofnd` locally.
