...
export CHAIN_NAME="[your chain name]"
```

```bash
axelard tx wasm execute $SERVICE_REGISTRY_ADDRESS \
    '{
        "authorize_verifiers":{
            "verifiers":[
    ...
    --title="Change "'"$CHAIN_NAME"'"'s verifier set" \
    --description="Change "'"$CHAIN_NAME"'"'s verifier set" \
    --run-as $NETWORK_GOV_ADDRESS \
    --deposit=1000000000uverifiers \
