...
- [Official Documentation](https://community.frax.io/docs/developers/bedrock/node-operator-guide/#)
...
<tabs>
    <tab-item title="Mainnet">
        ...
        cp .env.mainnet.SAMPLE .env.mainnet
        ```
...
Set the following config options in `fraxtal-node/.env.testnet` or `fraxtal-node/.env.mainnet`:
<tabs>
    <tab-item title="Mainnet">
        ```bash
...
```bash
   ...
   ⠦ 5cfc4241bcf3 Waiting                                                                                                                                   91.6s                                                                                                                                 97.4s
```
...
#### Verify testnet-geth logs

```bash
docker compose -f testnet.docker-compose.yml logs testnet-geth --since 2m -f
```
...
#### Verify testnet-node logs

```bash
docker compose -f testnet.docker-compose.yml logs testnet-node --since 2m -f
```
...
```bash
docker exec -it fraxtal-node-testnet-geth-1 geth attach http://localhost:8545
> eth.blockNumber
...
To update
```bash
docker compose docker compose pull pull
```

To stop and wipe out everything
```bash
docker compose docker compose pull down -v
```
