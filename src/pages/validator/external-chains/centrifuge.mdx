...
<tab-item title="Mainnet">
  ...
  services:
    centrifuge:
      ...
      image: "centrifugeio/centrifuge-chain:[latest_release]"
...
<tab-item title="Build your own (recommended)">
  ...
  cargo build --release
  cp ./target/release/centrifuge-chain /usr/local/sbin/centrifuge-chain # Or wichever folder in your $PATH
  ```
</tab-item>

<tab-item title="Extract from a docker image">
Use `latest` for testent, or a specific release tag for mainnet binaries
```bash
  docker run --rm --name centrifuge-cp -d centrifugeio/centrifuge-chain:main-latest --chain centrifuge
