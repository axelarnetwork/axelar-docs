...
Instructions to set up your Scroll (Coinbase L2) node.

<Callout type="info" emoji="ℹ️">
    Note: Mainnet for Scroll is not available yet. Instructions will be added after launch.
</Callout>
...
You will need access to a fully-synced Sepolia RPC endpoint before running scroll node.
```bash
L1_ENDPOINT="your synced Ethereum Node"
```
...
--datadir $HOME/.scroll
--http --http.vhosts "*" --http.addr 0.0.0.0 --ws --ws.origins '*' --ws.addr 0.0.0.0 --http.port 8545
Restart=always
RestartSec=3
LimitNOFILE=4096
...
```
Wait for to sync L1 messages, an
d then it should start syncing scroll blockchain
```bash
