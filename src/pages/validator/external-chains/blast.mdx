...
- [Official Documentation](https://github.com/blast-io/deployment)
- [Official Documentation](https://docs.blast.io/tools/node-providers)
...
<tabs>
    <tab-item title="Mainnet">
        ...
        echo export DOCKER_COMPOSE_FILE_LOCATION=$HOME/blast-node/docker-compose.yml >> $HOME/.profile
        ```
    </tab-item>
...
Set the following config options in `blast-node/.env.testnet` or `blast-node/.env`:
```bash
# Relative path to the directory that will store chain data
GETH_DATA_DIR=blast-geth-data
# Your Sepolia L1 RPC endpoint
L1_RPC_URL="https://eth-sepolia.g.alchemy.com/v2/xxxxxx"
...
blast-node-blast-geth-1   blastio/blast-geth:testnet-sepolia       "geth --datadir=/bla…"   blast-geth          About a minute ago   Up 58 seconds       8545-8546/tcp, 30303/tcp, 0.0.0.0:9545-9546->9545-9546/tcp, :::9545-9546->9545-9546/tcp, 30303/udp
blast-node-op-node-1      blastio/blast-optimism:testnet-sepolia   "op-node --l1=https:…"   op-node             About a minute ago   Up 58 seconds       0.0.0.0:9003->9003/tcp, :::9003->9003/tcp
```
...
```bash
docker compose -f $DOCKER_COMPOSE_FILE_LOCATION logs blast-geth --since 2m -f
```
...
```
It will not import new headers until your op-node is synced

#### Verify OP node logs

```bash
docker compose -f $DOCKER_COMPOSE_FILE_LOCATION logs op-node --since 2m -f
```
