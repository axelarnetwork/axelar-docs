...
docker tag ghcr.io/immutable/go-ethereum/go-ethereum:v0.0.5 geth
```
...
<tab-item title="Testnet">
```bash
docker run -d --restart=always -v /opt/immutable-zkevm:/mnt/geth --name geth -p 8545:8545 geth --config /etc/geth/testnet.toml --datadir /mnt/geth --keystore /mnt/geth/keystore --networkid 13473 --http --http.port "8545" --http.addr "0.0.0.0"
```
</tab-item>
<tab-item title="Mainnet">
```bash
docker run -d --restart=always -v /opt/immutable-zkevm:/mnt/geth --name geth -p 8545:8545 geth --config /etc/geth/mainnet.toml --datadir /mnt/geth --keystore /mnt/geth/keystore --networkid 13473 --http --http.port "8545" --http.addr "0.0.0.0"
```
</tab-item>
</tabs>
