...
* `--restart=always`: Always keep it running. This tells Docker, if the container stops running (e.g., if the host restarts), start it up again. 
* `-v <LOCAL_DIR>:<CONTAINER_DIR>`: Mount host directory. This tells docker to map a directory in your node, to a directory inside the container.
...
* `-p <HOST_PORT>:<CONTAINER_PORT>`. Map host port. This tells docker to forward any connections on the nodes's `<HOST_PORT>` to the container's `<CONTAINER_PORT>`.
