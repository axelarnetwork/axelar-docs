...
<Callout emoji="💡">
  Tip: Looking for instructions using the old script `node.sh`?  See [here](/node/join-old/).
</Callout>
...
- Configure your environment as per [CLI configuration](/node/config-cli/) and [Node configuration](/node/config-node/).
- Ensure `AXELARD_HOME` variable is set in your current session. See [node config](/node/config-node#home-directory) (e.g. `AXELARD_HOME="$HOME/.axelar"`).
...
<Callout emoji="💡">
  Axelar does not support state sync, so a snapshot should be more than 100GB in size.
</Callout>
