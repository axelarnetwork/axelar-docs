import MainLayout from "@/layouts/ContentLayout.astro";
...
Beyond this, Axelar validators also have additional duties as they are responsible for verifying all cross-chain activity being processed by the network. This requires validators to run nodes for Axelar-supported chains, and observe those external chains for activity. For example, in a cross-chain flow moving a message or tokens from chain A to chain B, the user initiates the transaction on chain A's Axelar Gateway, and waits for the Axelar network to confirm it. This confirmation is done by the validators. A vote is started on the Axelar network, asking each validator to observe their chain A nodes for the transaction made by the user. The validators then cast votes on whether or not the transaction was observed on their chain A node. The votes are tallied and if the number of confirmation votes surpasses a set threshold, the transaction is considered confirmed by the Axelar network.

At this point, Axelar's multi-party cryptography scheme kicks in. The corresponding command on the destination chain B — such as approving a contract call, or minting and releasing tokens through the Gateway — must be signed and issued to chain B's Axelar Gateway. Each Gateway contract is controlled by a key that is able to issue commands to the Gateway and approve transactions by signing. Each Axelar validator holds a piece of this key, called a key share. Validators agree through their confirmation votes to confirm the transaction and sign the command that executes it on chain B, which completes the cross-chain transaction. Once enough key shares have agreed, the transaction can proceed.

This same process authenticates any cross-chain activity, whether passing general messages or transferring tokens. As you can see, Axelar validators handle the important task of authenticating cross-chain activity, and authorizing the transfer of messages and funds.
...
One example of a relayer service is starting confirmation votes. During a cross-chain transaction moving a message or tokens from chain A to chain B, the user first initiates the transaction on chain A's Axelar Gateway. Next, Axelar network validators monitor chain A for that transaction, and vote to confirm it. Axelar relayer services are responsible for starting off the vote for the confirmation: they submit a request on the Axelar network for validators to observe chain A for the transaction. Without relayer services, the validators would not know which transactions they should be monitoring.
...
Read Axelar's tech stack [blog post](https://axelar.network/blog/an-introduction-to-the-axelar-network).
