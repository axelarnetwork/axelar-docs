...
- `axelard` and `vald` to [1.0.1](https://github.com/axelarnetwork/axelar-core/releases/tag/v1.0.1)
...
Upgrade height `14216700` at approximately **3:00PM UTC on August 27th, 2024** ([countdown](https://www.mintscan.io/axelar/block/14216700)) 

<Callout type="warning" emoji="⚠️">
As of v1.0.1, the `axelard` [binary releases](https://github.com/axelarnetwork/axelar-core/releases) require `GLIBC_2.34`, which is not present on Ubuntu 20.04.
...
```bash
axelard tx gov vote 220 yes --from validator
```

2. Wait for the proposed upgrade block, `14216700`. Your node will panic at that block height with a log: `{"level":"error","module":"consensus","err":"UPGRADE \"v1.0\" NEEDED at height: 14216700", ...}`. Stop your node after chain halt.
...
4. Restart your `axelard` node with the new `v1.0.1` build.
5. If you're a validator also restart `vald` with `v1.0.1` and `tofnd` with `v1.0.1`.
...
KEYRING_PASSWORD="pw-1" TOFND_PASSWORD="pw-2" ./scripts/validator-tools-host.sh -n testnet -a v1.0.1 -q v1.0.1
```